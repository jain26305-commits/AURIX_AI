...
export function useManufacturing() {
  const [activeTab, setActiveTab] = useState<
    'BOM_EXPLORER' | 'MRP_SCHEDULE' | 'WORK_CENTERS' | 'EXCEPTIONS'
  >('BOM_EXPLORER');

  const [selectedSkuId, setSelectedSkuId] =
    useState<string>('SKU-001');
  ...
  const {
    ...
    queryFn: ManufacturingService.fetchManufacturingReport,
    staleTime: 60_000,
  ...
  const activeBom =
    (data?.boms || []).find(
      (b: any) => b.parentSkuId === effectiveSkuId
    ) ||
    (data?.boms || [])[0] ||
    null;
